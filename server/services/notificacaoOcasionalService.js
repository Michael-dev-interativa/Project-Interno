...
async function dispararNotificacoesOcasionais() {
  ...
  console.log(`🔔 Criando notificações ocasionais — semana a partir de ${semanaInicio}`);
  ...
  for (const user of usuarios) {
    ...
    for (const atv of atvsDoUsuario) {
      try {
        ...
        await notificacaoModel.createNotificacao({
  ...
  const resultado = { totalCriados, totalJaExistiam, erros };
